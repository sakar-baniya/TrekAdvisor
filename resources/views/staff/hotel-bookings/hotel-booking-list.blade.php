...
@section('page-subtitle', 'Monitor all hotel reservation activity and requests.')
...
<x-layouts.dashboard>
    ...
    <section class="bg-white p-5 md:p-6 rounded-xl border border-slate-200 shadow-sm mb-8">
        <form method="GET" action="{{ route('staff.hotel-bookings.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <select name="hotel_id" class="bg-white border-slate-200 rounded-lg px-4 py-2.5 text-sm font-medium focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900/30 transition-all appearance-none cursor-pointer font-display">
    ...
    <div class="space-y-4 mb-10">
                    ...
                    <div class="flex items-center gap-5">
                        ...
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                ...
                                <h3 class="text-base font-semibold text-slate-900 leading-tight font-display">{{ $booking->hotelRoom?->hotel?->name ?? 'Hotel Booking' }}</h3>
                            </div>
                            <div class="flex flex-wrap items-center gap-x-4 gap-y-1.5 mt-1.5">
                                ...
                                <span class="flex items-center gap-2 text-xs text-slate-500">
                                    <i class="far fa-calendar text-slate-400"></i> {{ optional($booking->check_in)->format('M d') }} - {{ optional($booking->check_out)->format('M d, Y') }}
                                </span>
                    ...
                    <div class="flex items-center gap-4 border-t md:border-t-0 pt-4 md:pt-0">
                        @php
                            ...
                            $badgeStyle = match($statusVal) {
                                ...
                                'cancelled', 'cancellation_requested' => 'bg-red-50 text-red-700 border-red-100',
                        ...
                        <a href="{{ route('staff.hotel-bookings.show', $booking) }}" class="inline-flex items-center px-4 py-2 border border-slate-200 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-50 transition-colors">
                            Details
                        </a>
                    </div>
                </div>
        ...
        @empty
            <div class="bg-white p-12 rounded-xl border border-slate-200 text-center">
                <i class="fas fa-hotel text-4xl text-slate-100 mb-4 block"></i>
                <p class="text-sm font-medium text-slate-400">No hotel bookings found matching your criteria.</p>
            </div>
        @endforelse
    </div>
