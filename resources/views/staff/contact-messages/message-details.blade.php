<x-dashboard-layout>
    <x-slot name="header">
        <div class="admin-page-heading admin-page-heading--split">
            ...
            <a href="{{ route('staff.contact-messages.index') }}" class="admin-secondary-button">
        ...
        <article class="admin-panel">
            <div class="admin-panel__header">
                <div>
                    ...
                    <p>Sender information and full message content</p>
                </div>
            </div>
            ...
            <section class="admin-panel">
                <div class="admin-panel__header">
                    <div>
                        ...
                        <p>Save internal response handling note</p>
                    </div>
                </div>
                ...
                @if ($message->responded_at)
                    <div class="admin-info-list" style="margin-bottom: 1rem;">
                        ...
                        <div><span>By</span><strong>{{ $message->respondedByStaff?->name ?? 'Staff' }}</strong></div>
                    </div>
                @endif

                <form method="POST" action="{{ route('staff.contact-messages.respond', $message) }}" class="admin-status-form">
                    @csrf
                    @method('PATCH')
                    <label class="admin-field">
                        <span>Response Note</span>
                        <textarea name="staff_response" class="admin-input" rows="6" placeholder="Write your response or handling note">{{ old('staff_response', $message->staff_response) }}</textarea>
                    </label>
                    <button type="submit" class="admin-primary-button admin-primary-button--fit">Save Response</button>
                </form>
            </section>
        </aside>
    </section>
