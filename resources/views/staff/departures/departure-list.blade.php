...
@section('page-actions')
    <a href="{{ route('staff.departures.create') }}" class="inline-flex items-center px-4 py-2.5 bg-slate-900 text-white rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-slate-800 transition-all shadow-sm">
        Add Departure
    </a>
@endsection
...
<x-layouts.dashboard>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div class="bg-white border border-slate-200/70 rounded-2xl p-6">
            ...
            <form method="GET" action="{{ route('staff.departures.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
            ...
            @forelse ($departures as $dep)
                ...
                <div class="bg-white border border-slate-200/70 rounded-2xl p-5 hover:border-slate-300 hover:shadow-sm transition">
                    ...
                    <div class="flex gap-2 mt-3 pt-3 border-t border-slate-100">
                        <a href="{{ route('staff.departures.edit', $dep) }}" class="flex-1 bg-white border border-slate-200 hover:bg-slate-50 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-700 inline-flex items-center justify-center gap-2 transition-all">
                            <i class="fas fa-pen text-xs"></i> Edit Date
                        </a>
                        <a href="{{ route('staff.trek-bookings.index', ['departure_id' => $dep->id]) }}" class="bg-slate-900 text-white hover:bg-slate-800 rounded-xl px-3 py-2.5 inline-flex items-center justify-center transition-all shadow-sm">
