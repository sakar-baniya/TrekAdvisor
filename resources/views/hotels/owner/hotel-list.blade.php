<x-layouts.dashboard>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-10">
        <div>
            <h1 class="text-3xl font-semibold text-slate-900 tracking-tight">Property Management</h1>
        ...
        <a href="{{ route('hotel_owner.hotels.create') }}" class="inline-flex items-center px-6 py-3 bg-slate-900 text-white rounded-xl text-xs font-semibold uppercase tracking-widest hover:bg-slate-800 transition-colors shadow-lg shadow-slate-900/20">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                 ...
                 <div class="relative h-48 bg-slate-900 overflow-hidden">
                      ...
                      <div class="absolute bottom-6 left-8">
                           ...
                           <div class="flex items-center gap-2 mb-2">
                                ...
                                <span class="text-[9px] font-semibold text-white uppercase tracking-[0.2em]">{{ $hotel->status }}</span>
                           </div>
                           <h3 class="text-xl font-semibold text-white tracking-tight line-clamp-1 italic italic">{{ $hotel->name }}</h3>
                      </div>
                 </div>
                 ...
                 <div class="p-8 flex-grow flex flex-col">
                      <div class="flex flex-wrap gap-4 mb-8">
                           <div class="flex flex-col">
                                <span class="text-[8px] font-semibold text-slate-400 uppercase tracking-widest">Location</span>
                                <span class="text-[10px] font-semibold text-slate-900 uppercase italic">{{ $hotel->location }}</span>
                           </div>
                           <div class="w-px h-6 bg-slate-100 hidden sm:block"></div>
                           <div class="flex flex-col">
                                <span class="text-[8px] font-semibold text-slate-400 uppercase tracking-widest">Inventory</span>
                                <span class="text-[10px] font-semibold text-slate-900 uppercase italic">{{ $hotel->rooms_count ?? 0 }} Room Types</span>
                           </div>
                      </div>
                      ...
                      <div class="mt-auto pt-6 border-t border-slate-50 flex items-center justify-between">
                           <div class="flex items-center gap-2">
                                ...
                                <span class="text-[10px] font-semibold text-slate-400 uppercase tracking-tighter">{{ $hotel->gallery_count ?? 0 }} Photos</span>
                           </div>
                           <a href="{{ route('hotel_owner.hotels.edit', $hotel) }}" class="inline-flex items-center px-5 py-2.5 bg-slate-900 text-white text-[9px] font-semibold uppercase tracking-widest rounded-xl hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/10">
                                Edit Property
                           </a>
                      </div>
                 </div>
        ...
        @empty
            <div class="col-span-full py-20 px-6 text-center bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100 shadow-sm">
                ...
                <h3 class="text-xl font-semibold text-slate-900 tracking-tight">No hotels registered</h3>
                ...
                <div class="mt-10">
                    <a href="{{ route('hotel_owner.hotels.create') }}" class="inline-flex items-center px-10 py-4 bg-slate-900 text-white text-xs font-semibold uppercase tracking-widest rounded-2xl hover:bg-slate-800 transition-all shadow-2xl shadow-slate-900/20">
