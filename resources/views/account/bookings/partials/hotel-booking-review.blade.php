    ...
    @if ($booking->status === 'completed')
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500">Share your stay experience with other travelers.</p>
            </div>
        </div>
        ...
        @else
            @if ($booking->hotelRoom?->hotel)
                <x-reviews.form :action="route('account.reviews.hotels.store', $booking->hotelRoom->hotel)" submit-label="Submit Stay Review" />
            @endif
        @endif
    @else
        <div class="flex items-center gap-4 text-slate-400">
            ...
            <p class="text-xs text-slate-500 italic">Review will be available once your stay is completed.</p>
        </div>
    @endif
</div>
