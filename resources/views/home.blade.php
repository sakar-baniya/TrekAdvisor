<x-app-layout>
    ...
    <section class="relative min-h-[85vh] flex items-center justify-center bg-slate-900 overflow-hidden">
        ...
        <div class="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center py-20 text-center">
            <div class="mb-12 max-w-4xl">
                <h1 class="text-white text-5xl md:text-6xl font-semibold tracking-tight mb-8 drop-shadow-2xl leading-tight">
            ...
            <div class="w-full max-w-2xl" x-data="{
                ...
                <form :action="actionUrl()" method="GET" class="bg-white p-2.5 rounded-full shadow-lg flex items-center w-full focus-within:ring-4 focus-within:ring-white/10 transition-all">
                    ...
                    <button type="submit" class="bg-slate-900 text-white font-semibold px-10 py-4 rounded-full hover:bg-slate-800 transform transition-all hover:scale-[1.01] shadow-md flex-shrink-0 uppercase text-xs tracking-widest" x-text="exploreLabel()"></button>
                </form>
            </div>
        </div>
    </section>
    ...
    <section class="py-24 bg-slate-50 border-t border-slate-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row md:items-end md:justify-between mb-16 border-b border-slate-200 pb-8">
                <div>
                    <p class="text-slate-400 font-semibold tracking-[0.2em] uppercase text-[10px] mb-2">Adventure Awaits</p>
                    <h2 class="text-4xl md:text-5xl font-semibold text-slate-900 tracking-tight">Featured Treks</h2>
                ...
                @forelse ($featuredTreks->take(3) as $trek)
                    <article class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-all duration-300 flex flex-col h-full group">
                        <div class="h-64 overflow-hidden relative bg-slate-200">
                            ...
                            @endif
                            <div class="absolute top-6 right-6 bg-white/95 backdrop-blur-sm text-slate-900 text-[10px] font-semibold uppercase tracking-widest px-4 py-2 rounded-xl shadow-sm flex items-center gap-2">
                                <i class="fas fa-star text-amber-500"></i> {{ $trek->reviews_avg_rating ? number_format($trek->reviews_avg_rating, 1) : 'NEW' }}
                            </div>
                        </div>
                        <div class="p-6 flex flex-col flex-grow">
                            <div class="flex items-center gap-6 text-[10px] font-semibold text-slate-400 mb-4 uppercase tracking-[0.15em]">
    ...
    <section id="featured-hotels" class="py-24 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row md:items-end md:justify-between mb-16 border-b border-slate-50 pb-8">
                <div>
                    <p class="text-slate-400 font-semibold tracking-[0.2em] uppercase text-[10px] mb-2">Luxury Stays</p>
                    <h2 class="text-4xl md:text-5xl font-semibold text-slate-900 tracking-tight">Featured Hotels</h2>
                ...
                @forelse ($featuredHotels->take(3) as $hotel)
                    <article class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-all duration-300 flex flex-col h-full group">
                        ...
                        <div class="p-6 flex flex-col flex-grow">
                            ...
                            <div class="flex items-center text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-6">
    ...
    <section class="py-32 relative overflow-hidden bg-slate-900 text-white text-center rounded-t-3xl mt-16 group">
        ...
        <div class="relative z-10 max-w-4xl mx-auto px-6">
            <h2 class="text-5xl md:text-7xl font-semibold tracking-tight mb-8 leading-tight text-white">Ready for Your Adventure?</h2>
    ...
    <section class="py-24 bg-slate-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-20">
                <p class="text-slate-400 font-semibold tracking-[0.2em] uppercase text-[10px] mb-3">Testimonials</p>
                <h2 class="text-4xl md:text-5xl font-semibold text-slate-900 tracking-tight">What Trekkers Say</h2>
            </div>
                ...
                @foreach($testimonials as $testimonial)
                    <article class="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex flex-col h-full hover:shadow-md transition-all duration-200">
                        ...
                        <div class="flex items-center gap-5 pt-8 border-t border-slate-50 mt-auto">
                            <div class="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center text-slate-900 font-semibold flex-shrink-0 text-lg">
                                {{ substr($testimonial->user?->name ?? 'A', 0, 1) }}
                            </div>
                            <div>
                                <strong class="block text-slate-900 font-semibold text-xs uppercase tracking-widest mb-1">{{ $testimonial->user?->name ?? 'Anonymous' }}</strong>
