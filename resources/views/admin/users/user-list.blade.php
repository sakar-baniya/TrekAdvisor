<x-dashboard-layout>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-10">
        <div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Identity & Access</h1>
            <p class="text-slate-500 font-medium mt-1">Manage user roles, partner approvals, and platform permissions.</p>
        </div>
        <a href="{{ route('admin.users.create-staff') }}" class="inline-flex items-center px-6 py-3 bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-800 transition-colors shadow-lg shadow-slate-900/20">
    ...
    @if (session('error'))
        <div class="mb-8 p-4 bg-red-50 border-l-4 border-red-500 rounded-r-xl">
            <p class="text-[10px] font-black text-red-800 uppercase tracking-widest">Error</p>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden mb-10">
        <div class="px-8 py-6 border-b border-slate-50 bg-slate-50/20">
            <h3 class="text-xs font-black text-slate-900 uppercase tracking-widest">Search & Segmentation</h3>
        </div>
        <div class="p-8">
            <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-col lg:flex-row lg:items-end gap-6">
                ...
                <button type="submit" class="w-full lg:w-auto px-8 py-3.5 bg-slate-100 text-slate-600 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-900 hover:text-white transition-all">
                    Apply List Filters
                </button>
            </form>
        </div>
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                <thead>
                    <tr class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 border-b border-slate-50 bg-slate-50/20">
                        ...
                        <tr class="group hover:bg-slate-50/50 transition-colors">
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    <div class="w-10 h-10 rounded-full bg-slate-900 text-white flex items-center justify-center font-black text-xs shrink-0">
                                        {{ substr($user->name, 0, 1) }}
                                    </div>
                                    <div>
                                        <div class="text-sm font-black text-slate-900 group-hover:text-emerald-600 transition-colors uppercase tracking-tight">{{ $user->name }}</div>
                                        <div class="text-[10px] font-medium text-slate-400 lowercase tracking-wider">{{ $user->email }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                                <span class="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border border-slate-100 bg-slate-50 text-slate-600">
                            ...
                            <td class="px-8 py-6">
                                @if ($user->role === 'hotel_owner')
                                    ...
                                    <div class="flex items-center gap-2">
                                         ...
                                         <span class="text-[9px] font-black uppercase tracking-widest {{ $approved ? 'text-emerald-700' : 'text-amber-700' }}">
                                             {{ $approved ? 'Approved' : 'Pending Verification' }}
                                         </span>
                                    </div>
                                @else
                                    <span class="text-[9px] font-black text-slate-300 uppercase italic">N/A</span>
                                @endif
                            </td>
                            <td class="px-8 py-6">
                                ...
                                <div class="flex items-center justify-end gap-4">
                                    @if ($user->role === 'hotel_owner' && $user->approval_status !== 'approved')
                                        <form method="POST" action="{{ route('admin.users.approve', $user) }}">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="px-4 py-2 bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-emerald-600 hover:text-white transition-all shadow-sm">
                                                Approve
                                            </button>
                                        </form>
                                    @endif
                                    ...
                                    <form method="POST" action="{{ route('admin.users.role', $user) }}" class="flex items-center gap-2">
                                        @csrf
                                        @method('PATCH')
                                        <select name="role" class="text-[10px] font-black uppercase border-slate-100 rounded-xl py-2 pl-3 pr-8 focus:ring-slate-900 focus:border-slate-900 bg-slate-50/50">
