...
@section('page-back', route('admin.trek-bookings.index'))
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            @forelse ($booking->passengers as $passenger)
                                <tr>
                                    <td class="px-8 py-5 text-sm font-semibold text-slate-900">{{ $passenger->full_name }}</td>
                                    <td class="px-8 py-5 text-sm font-semibold text-slate-600">{{ $passenger->age }}</td>
                                    <td class="px-8 py-5 text-sm font-mono font-semibold text-slate-400">{{ $passenger->passport_number ?: '---' }}</td>
                                </tr>
                            @empty
                                <tr>
        ...
        <!-- Sidebar Stats -->
         <aside class="space-y-8">
            ...
            <section class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                ...
                <form method="POST" action="{{ route('admin.trek-bookings.status', $booking) }}" class="space-y-4">
                    @csrf
                    @method('PATCH')
