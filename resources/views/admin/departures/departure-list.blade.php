<x-dashboard-layout>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-10">
        <div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Trek Operations</h1>
            <p class="text-slate-500 font-medium mt-1">Manage departure dates, prices, and capacity for all expeditions.</p>
        </div>
        <a href="{{ route('admin.departures.create') }}" class="inline-flex items-center px-6 py-3 bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-800 transition-colors shadow-lg shadow-slate-900/20">
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden mb-10">
        <div class="px-8 py-6 border-b border-slate-50 bg-slate-50/20">
             <h3 class="text-xs font-black text-slate-900 uppercase tracking-widest leading-none">Global Filters</h3>
        </div>
        <div class="p-8">
            <form method="GET" action="{{ route('admin.departures.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 items-end">
                <div>
                    <x-input-label value="Departure Trek" class="mb-2 text-[10px] font-black uppercase tracking-widest text-slate-400" />
                ...
                <div>
                    <x-input-label value="Booking Status" class="mb-2 text-[10px] font-black uppercase tracking-widest text-slate-400" />
                ...
                <div>
                    <x-input-label value="Calendar Month" class="mb-2 text-[10px] font-black uppercase tracking-widest text-slate-400" />
                ...
                <button type="submit" class="px-8 py-3.5 bg-slate-100 text-slate-600 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-900 hover:text-white transition-all">
                    Search Dates
                </button>
            </form>
        </div>
        ...
        @forelse ($departures as $dep)
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 overflow-hidden flex flex-col italic-hover">
                 <div class="p-8 pb-4">
                     <div class="flex items-center justify-between gap-4 mb-4">
                        <div class="flex items-center gap-2">
                             ...
                             <span class="text-[9px] font-black uppercase tracking-widest text-slate-400 italic italic">{{ $dep->status }}</span>
                        </div>
                        <span class="text-[9px] font-black text-slate-900 bg-slate-50 px-2 py-1 rounded-lg">#{{ $dep->id }}</span>
                     </div>
                     <h3 class="text-xl font-black text-slate-900 tracking-tight leading-tight line-clamp-2 min-h-[3.5rem]">{{ $dep->trek?->title ?? 'Trek Departure' }}</h3>
                     <p class="text-[10px] font-black text-emerald-600 uppercase tracking-widest mt-2">
                 ...
                 <div class="px-8 py-6 bg-slate-50/50 grid grid-cols-2 gap-y-4 gap-x-6 border-y border-slate-50">
                      <div>
                           <span class="block text-[8px] font-black text-slate-400 uppercase tracking-widest">Price</span>
                           <strong class="text-xs font-black text-slate-900 tracking-tight">NPR {{ number_format($dep->price, 0) }}</strong>
                      </div>
                      <div>
                           <span class="block text-[8px] font-black text-slate-400 uppercase tracking-widest">Available</span>
                           <strong class="text-xs font-black text-slate-900 tracking-tight">{{ max($dep->capacity - $dep->booked_seats, 0) }} / {{ $dep->capacity }} Seats</strong>
                      </div>
                 </div>

                 <div class="p-8 flex items-center justify-between gap-4">
                      <a href="{{ route('admin.departures.edit', $dep) }}" class="flex-1 inline-flex justify-center items-center py-3 bg-white border border-slate-200 text-slate-600 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-50 transition-all shadow-sm">
