<x-dashboard-layout>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-10">
        <div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Trek Inventory</h1>
            <p class="text-slate-500 font-medium mt-1">Monitor and manage all trekking expeditions across the marketplace.</p>
        </div>
        <a href="{{ route('admin.treks.create') }}" class="inline-flex items-center px-6 py-3 bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-800 transition-colors shadow-lg shadow-slate-900/20">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden mb-8 p-8">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h3 class="text-xs font-black text-slate-900 uppercase tracking-widest">Advanced Filters</h3>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
            ...
            <div class="group bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 overflow-hidden flex flex-col italic-hover">
                 ...
                 <div class="relative h-48 overflow-hidden">
                      ...
                      <div class="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-slate-900/80">
                           <span class="px-2 py-1 rounded-md bg-white/20 backdrop-blur-md text-[9px] font-black text-white uppercase tracking-widest">
                               {{ $trek->difficulty }}
                           </span>
                      </div>
                 </div>
                 ...
                 <div class="p-6 flex-grow flex flex-col justify-between">
                      <div>
                           <div class="flex items-center justify-between gap-4 mb-2">
                                <h3 class="text-lg font-black text-slate-900 leading-tight line-clamp-1 group-hover:text-emerald-600 transition-colors">{{ $trek->title }}</h3>
                                <span class="px-2 py-0.5 rounded-full border border-slate-100 text-[8px] font-black uppercase tracking-tighter text-slate-400">
                                    {{ $trek->status }}
                                </span>
                           </div>
                      ...
                      <div class="flex items-center justify-between pt-6 border-t border-slate-50 mt-4">
                           <div class="flex flex-col">
                                <span class="text-[8px] font-black text-slate-400 uppercase tracking-widest">Starting Price</span>
                                <strong class="text-sm font-black text-slate-900 italic tracking-tight italic">NPR {{ number_format($trek->base_price ?? 0, 0) }}</strong>
                           </div>
        ...
        @empty
            <div class="col-span-full py-20 px-6 text-center bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100 shadow-sm">
                ...
                <h3 class="text-xl font-black text-slate-900 tracking-tight">No treks found</h3>
                ...
                <div class="mt-10">
                    <a href="{{ route('admin.treks.index') }}" class="text-xs font-black text-slate-900 uppercase tracking-widest underline underline-offset-8 decoration-slate-200 hover:decoration-slate-900 transition-all">
                        Clear All Filters
                    </a>
                </div>
            </div>
    ...
    @if ($treks->hasPages())
        <div class="flex flex-col sm:flex-row items-center justify-between border-t border-slate-100 pt-8 pb-12">
            <div class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 sm:mb-0 italic italic">
