...
@if ($errors->any())
    <div class="mb-8 p-4 bg-red-50 border-l-4 border-red-500 rounded-r-xl">
        <p class="text-sm font-semibold text-red-800 uppercase tracking-widest">Action Required</p>
...
<div class="space-y-12" x-data="{
    ...
    <section class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-8 md:p-10 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h3 class="text-xl font-semibold text-slate-900 tracking-tight">Trek Media</h3>
        ...
        <div class="p-8 md:p-10">
            ...
            <p class="text-[10px] font-semibold text-slate-400 uppercase tracking-widest leading-relaxed">
    ...
    <section class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden text-black">
        <div class="p-8 md:p-10 border-b border-slate-50">
            <h3 class="text-xl font-semibold text-slate-900 tracking-tight">Full Description *</h3>
    ...
    <section class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-8 md:p-10 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h3 class="text-xl font-semibold text-slate-900 tracking-tight">Day-by-Day Itinerary</h3>
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Add as many days as needed for the trek</p>
            </div>
            <button type="button" @click="addDay" class="inline-flex items-center px-6 py-3 bg-emerald-50 text-emerald-600 text-[10px] font-semibold uppercase tracking-widest rounded-xl hover:bg-emerald-600 hover:text-white transition-all shadow-sm">
            ...
            <template x-for="(day, index) in itinerary" :key="index">
                <div class="p-8 bg-slate-50/50 rounded-3xl border border-slate-100 group relative">
                    <div class="flex items-center justify-between mb-6">
                        <span class="text-xs font-semibold text-slate-900 uppercase tracking-widest">
                            Day <span x-text="index + 1"></span>
                        </span>
                        <button type="button" @click="removeDay(index)" class="text-[10px] font-semibold text-red-400 hover:text-red-700 uppercase tracking-widest transition-colors">
                            Remove
                        </button>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            <x-input-label value="Title" class="mb-2 text-[10px] font-semibold uppercase tracking-widest text-slate-400" />
                            <x-text-input type="text" ::name="'itinerary['+index+'][title]'" x-model="day.title" placeholder="e.g. Flight to Lukla & Trek to Phakding" />
                        </div>
                        <div>
                            <x-input-label value="Description" class="mb-2 text-[10px] font-semibold uppercase tracking-widest text-slate-400" />
