<x-dashboard-layout>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10">
        <div>
            <h1 class="text-3xl font-semibold text-slate-900 tracking-tight">Admin Dashboard</h1>
            <p class="text-slate-500 font-medium">Real-time overview of the TrekAdvisor marketplace.</p>
        </div>
        <div class="flex gap-3">
            <button class="inline-flex items-center px-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-semibold uppercase tracking-widest text-slate-600 hover:bg-slate-50 transition-colors shadow-sm">
                <i class="fas fa-file-export mr-2"></i> Export
            </button>
            <a href="{{ route('admin.treks.create') }}" class="inline-flex items-center px-4 py-2 bg-slate-900 text-white rounded-xl text-xs font-semibold uppercase tracking-widest hover:bg-slate-800 transition-colors shadow-lg shadow-slate-900/20">
        ...
        @foreach([
            ...
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-[10px] font-semibold uppercase tracking-widest px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800">
                        {{ $stat['trend'] }}
                    </span>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
        ...
        <div class="lg:col-span-2 bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h3 class="text-lg font-semibold text-slate-900 tracking-tight">Revenue Trend</h3>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Last 6 Months Growth</p>
                </div>
                <select class="bg-slate-50 border-none text-xs font-semibold uppercase tracking-widest rounded-xl px-4 py-2 text-slate-600">
        ...
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
            <h3 class="text-lg font-semibold text-slate-900 tracking-tight mb-8">Booking Status</h3>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
                ...
                <a href="#" class="text-[10px] font-semibold text-slate-400 uppercase tracking-widest hover:text-slate-900 transition-colors">View All &rarr;</a>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @foreach($recentBookings ?? [] as $booking)
                            <tr class="group hover:bg-slate-50/50 transition-colors">
                                <td class="px-8 py-5">
                                    <div class="text-sm font-semibold text-slate-900 mb-1 group-hover:text-emerald-600 transition-colors">{{ $booking->title }}</div>
                                ...
                                <td class="px-8 py-5">
                                    <span class="text-sm font-semibold text-slate-900 tracking-tight">NPR {{ number_format($booking->amount, 0) }}</span>
                                </td>
                                <td class="px-8 py-5">
                                    @php
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white rounded-3xl border-l-[6px] border-amber-400 shadow-sm p-8 group">
                <div class="flex items-center justify-between mb-8">
                    <h3 class="text-lg font-semibold text-slate-900 tracking-tight">Pending Approval</h3>
                    <span class="px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 text-[10px] font-semibold">{{ $stats['pending_hotels'] ?? 0 }}</span>
                </div>
                <div class="flex items-start gap-5">
                    ...
                    <div>
                        <h4 class="text-sm font-semibold text-slate-900 mb-2 uppercase tracking-tight">Hotel Review Queue</h4>
                        ...
                        <a href="{{ route('admin.hotels.index', ['status' => 'Pending']) }}" class="inline-flex items-center justify-center w-full py-3 bg-slate-100 text-slate-600 text-[10px] font-semibold rounded-xl hover:bg-slate-900 hover:text-white transition-all uppercase tracking-widest">
                            Review Queue
                        </a>
                    </div>
                </div>
            ...
            <!-- Admin Tools -->
            <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
                <h3 class="text-lg font-semibold text-slate-900 tracking-tight mb-2">Platform Tools</h3>
                ...
                <div class="grid grid-cols-1 gap-3">
                    <a href="{{ route('admin.users.index') }}" class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-transparent hover:border-slate-100 hover:bg-white transition-all group">
                        ...
                        <span class="text-xs font-semibold text-slate-600 uppercase tracking-widest group-hover:text-slate-900 transition-colors">User Access</span>
                    </a>
                    <a href="{{ route('admin.payments.index') }}" class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-transparent hover:border-slate-100 hover:bg-white transition-all group">
                        ...
                        <span class="text-xs font-semibold text-slate-600 uppercase tracking-widest group-hover:text-slate-900 transition-colors">Finance Logs</span>
                    </a>
                </div>
            </div>
        </div>
