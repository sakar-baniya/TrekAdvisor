...
$files = foreach($p in $paths){
    if(Test-Path $p){ Get-ChildItem $p -Recurse -File -Filter *.php }
}
    ...
    for($i=0; $i -lt $lines.Count; $i++){
        if($lines[$i] -match '^\s*(public|protected)\s+function\s+([A-Za-z_][A-Za-z0-9_]*)\s*\('){
            ...
            for($j=$i-1; $j -ge 0; $j--){
                ...
                if($lines[$j] -match '^\s*\{\s*$|^\s*\}\s*$'){ break }
            }
            if($start -lt 0){ continue }

            for($k=$start+1; $k -lt $i; $k++){
            ...
            for($d = $doc.Count - 1; $d -ge 0; $d--){
                $lines.Insert($start, [string]$doc[$d])
            }

            $delta = $doc.Count - $removeCount
            $i += $delta
            $changed = $true
        }
    }
